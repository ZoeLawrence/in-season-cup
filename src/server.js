    ...
    switch (interaction.data.name.toLowerCase()) {
      ...
      case NEXT_GAME_COMMAND.name.toLowerCase(): {
        ...
        return new JsonResponse({
          ...
          data: {
            flags: InteractionResponseFlags.IS_COMPONENTS_V2,
            content: `${JSON.stringify(channel)} ${d.toISOString()}`,
          },
        });
      }
...
async function assignTeams(results, champion, request, env) {
  ...
  const stmt = env.ASSIGN_DB.prepare(
    'INSERT INTO players (team, user_id, isChamp) VALUES (?, ?, ?);',
  );
...
async function createFirstMatch(game_id, game_time, team, env) {
  const { results } = await env.ASSIGN_DB.prepare(
    'INSERT INTO matchup (gameid, datetime, team) VALUES (?, ?, ?);',
  )
