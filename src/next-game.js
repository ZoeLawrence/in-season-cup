...
export async function testAssignments(env) {
  ...
  if (currentMatch[0] != undefined) {
    ...
    if (current_time.getTime() > game_time.getTime()) {
      ...
      const game_data = await getNHLData(
        `gamecenter/${currentMatch[0].gameid}/landing`,
      );
      ...
      const stmt = env.ASSIGN_DB.prepare(
        'UPDATE players SET isChamp = ? WHERE team = ?;',
      );
      ...
      title = `game time`;
      description = `<t:${game_time.getTime() / 1000}:F>`;
    }
  } else {
...
async function getNewChamp(env) {
  const { results } = await env.ASSIGN_DB.prepare(
    'SELECT * FROM players WHERE isChamp = 1;',
  ).run();
  return results;
}
...
async function getNextMatch(game_id, game_time, team, env) {
  const { results } = await env.ASSIGN_DB.prepare(
    'UPDATE matchup SET gameid = ?, datetime = ?, team = ? WHERE rowid = 1;',
  )
